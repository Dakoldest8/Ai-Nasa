...
DESKTOP_TEST_MODE = "--desktop-test" in sys.argv or os.getenv("PI_DESKTOP_TEST", "0") == "1"
...
CONFIG_FILE = local_path("config", "config.json")
MEMORY_FILE = local_path("memory.json")
BMO_IMAGE_FILE = local_path("current_image.jpg")
WAKE_WORD_MODEL = local_path("models", "wakeword.onnx")
WAKE_WORD_THRESHOLD = 0.5
WHISPER_BIN = local_path("whisper.cpp", "build", "bin", "whisper-cli")
WHISPER_MODEL = local_path("whisper.cpp", "models", "ggml-base.en.bin")
FACES_DIR = local_path("faces")
SOUNDS_DIR = local_path("sounds")
...
DEFAULT_CONFIG = {
    ...
    "voice_model": local_path("piper", "en_GB-semaine-medium.onnx"),
    "chat_memory": True,
    "camera_rotation": 0,
...
TEXT_MODEL = os.getenv("PI_TEXT_MODEL", CURRENT_CONFIG["text_model"])
VISION_MODEL = os.getenv("PI_VISION_MODEL", CURRENT_CONFIG["vision_model"])
...
# Sound Directories
greeting_sounds_dir = local_path("sounds", "greeting_sounds")
ack_sounds_dir = local_path("sounds", "ack_sounds")
thinking_sounds_dir = local_path("sounds", "thinking_sounds")
error_sounds_dir = local_path("sounds", "error_sounds")
...
class BotGUI:
    ...
    def __init__(self, master):
        ...
        self.exiting = False
        self.desktop_test_mode = DESKTOP_TEST_MODE
        self.desktop_input_var = tk.StringVar()
        self.warned_runtime = set()
        self.runtime_status = {}
        ...
        self.oww_model = None
        if self.desktop_test_mode:
            print("[INIT] Desktop test mode: wake word disabled.", flush=True)
        elif Model is None:
            print("[INIT] openwakeword not installed. Falling back to push-to-talk only.", flush=True)
        elif os.path.exists(WAKE_WORD_MODEL):
            try:
        ...
        self.runtime_status, self.runtime_warnings = self.collect_runtime_status()
        ...
        self.desktop_entry = ttk.Entry(master, textvariable=self.desktop_input_var, font=('Arial', 12))
        self.desktop_send_button = ttk.Button(master, text="Send Test Prompt", command=self.submit_desktop_prompt)
        self.desktop_image_button = ttk.Button(master, text="Test Image", command=self.submit_desktop_image)
        ...
        if self.desktop_test_mode:
            ...
            self.desktop_entry.place(relx=0.5, rely=0.68, anchor=tk.S, relwidth=0.55)
            self.desktop_send_button.place(relx=0.82, rely=0.68, anchor=tk.S)
            self.desktop_image_button.place(relx=0.82, rely=0.62, anchor=tk.S)
            self.desktop_entry.bind('<Return>', self.submit_desktop_prompt)
            threading.Thread(target=self.start_desktop_test_mode, daemon=True).start()
        else:
    ...
    def start_desktop_test_mode(self):
        self.warm_up_logic()
        self.tts_active.set()
        self.tts_thread = threading.Thread(target=self._tts_worker, daemon=True)
        self.tts_thread.start()
        self.tts_active.clear()
        self.set_state(BotStates.IDLE, "Desktop demo ready. Type a prompt below.")

    def submit_desktop_prompt(self, event=None):
        if not self.desktop_test_mode:
            return

        prompt = self.desktop_input_var.get().strip()
        if not prompt:
            return

        self.desktop_input_var.set("")
        self.append_to_text(f"YOU: {prompt}")
        self.interrupted.clear()
        threading.Thread(target=self.chat_and_respond, args=(prompt, None), daemon=True).start()

    def submit_desktop_image(self):
        if not self.desktop_test_mode:
            return

        image_path = filedialog.askopenfilename(
            title="Choose an image for vision test",
            filetypes=[("Image Files", "*.png;*.jpg;*.jpeg;*.webp;*.bmp"), ("All Files", "*.*")]
        )
        if not image_path:
            return

        prompt = self.desktop_input_var.get().strip() or "What do you see in this image?"
        self.append_to_text(f"YOU: {prompt} [image: {os.path.basename(image_path)}]")
        self.interrupted.clear()
        threading.Thread(target=self.chat_and_respond, args=(prompt, image_path), daemon=True).start()
        ...
        for state in states:
            ...
            if not self.animations[state]:
                if self.animations.get("idle"):
                    ...
                    # Blue screen fallback when no face assets are available.
    ...
    def collect_runtime_status(self):
        piper_binary = local_path("piper", "piper.exe") if os.name == "nt" else local_path("piper", "piper")
        voice_model = CURRENT_CONFIG.get("voice_model", local_path("piper", "en_GB-semaine-medium.onnx"))
        if not os.path.isabs(voice_model):
            voice_model = local_path(voice_model)

        status = {
            "wakeword_model": os.path.exists(WAKE_WORD_MODEL),
            "whisper_bin": os.path.exists(WHISPER_BIN),
            "whisper_model": os.path.exists(WHISPER_MODEL),
            "piper_binary": os.path.exists(piper_binary),
            "voice_model": os.path.exists(voice_model),
            "faces_dir": os.path.isdir(FACES_DIR),
            "sounds_dir": os.path.isdir(SOUNDS_DIR),
            "sounddevice": sd is not None,
        }

        warnings_list = []
        if not status["wakeword_model"]:
            warnings_list.append(f"Wake word model missing: {WAKE_WORD_MODEL}")
        if not status["whisper_bin"] or not status["whisper_model"]:
            warnings_list.append("Whisper.cpp missing (wake/ptt transcription will not work).")
        if not status["piper_binary"] or not status["voice_model"]:
            warnings_list.append("Piper missing (spoken output disabled).")
        if not status["faces_dir"]:
            warnings_list.append("faces/ not found (GUI will use fallback visuals).")
        if not status["sounds_dir"]:
            warnings_list.append("sounds/ not found (UI sound cues disabled).")
        if not status["sounddevice"]:
            warnings_list.append("sounddevice not available (mic/audio disabled).")

        return status, warnings_list

    def report_runtime_status(self):
        print("[CHECK] Runtime dependency status:", flush=True)
        for key, value in self.runtime_status.items():
            print(f"[CHECK] {key}: {'OK' if value else 'MISSING'}", flush=True)

        if self.runtime_warnings:
            for warning_text in self.runtime_warnings:
                print(f"[WARN] {warning_text}", flush=True)
    ...
    def detect_wake_word_or_ptt(self):
        ...
        if sd is None:
            self.set_state(BotStates.ERROR, "sounddevice not installed.")
            self.ptt_event.wait()
            self.ptt_event.clear()
            return "PTT"
    ...
    def transcribe_audio(self, filename):
        ...
        if not os.path.exists(WHISPER_BIN) or not os.path.exists(WHISPER_MODEL):
            if "whisper_missing" not in self.warned_runtime:
                print(f"[WARN] Missing Whisper binary/model at {WHISPER_BIN} and/or {WHISPER_MODEL}", flush=True)
                self.warned_runtime.add("whisper_missing")
            return ""
        try:
    ...
    def speak(self, text):
        ...
        if sd is None or not os.path.exists(piper_binary) or not os.path.exists(voice_model):
            if "piper_missing" not in self.warned_runtime:
                print(f"[AUDIO] Piper/audio stack not available. Expected binary: {piper_binary}", flush=True)
                print(f"[AUDIO] Expected voice model: {voice_model}", flush=True)
                self.warned_runtime.add("piper_missing")
            time.sleep(min(max(len(clean) * 0.02, 0.4), 2.0))
            return
